...
export class OrganizationsService {
  ...
  async create(command: CreateOrganizationCommand): Promise<OrganizationResult> {
    ...
    return translateRepositoryConflict(() =>
      this.repository.createOrganization({
        id,
  ...
  async get(command: GetOrganizationCommand): Promise<OrganizationResult> {
    return this.selectOrganization(command.slug);
  }
  ...
  async createInvitation(
    ...
    const invitation = await translateRepositoryConflict(() =>
...
async function translateRepositoryConflict<T>(operation: () => Promise<T>): Promise<T> {
  try {
    return await operation();
  } catch (error) {
    if (error instanceof OrganizationRepositoryError && error.kind === 'conflict') {
      throw new OrganizationsError('conflict', { cause: error });
    }
    throw error;
  }
}
